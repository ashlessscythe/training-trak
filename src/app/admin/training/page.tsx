...
export default function AdminTrainingPage() {
  ...
  const [trainingData, setTrainingData] = useState<Record<string, TrainingWithRelations[]>>({});
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 mb-8">
        {sites.map(site => {
          ...
          return (
            <Card 
              key={site.id} 
              className={`hover:shadow-md transition-shadow cursor-pointer ${selectedSite === site.id ? 'ring-2 ring-primary' : ''}`}
              onClick={() => setSelectedSite(selectedSite === site.id ? null : site.id)}
            >
              <CardHeader className="pb-2">
                <CardTitle>{site.name}</CardTitle>
                <p className="text-sm text-muted-foreground">Site Code: {site.code}</p>
              </CardHeader>
              <CardContent>

...
import { User, SOP, Position } from "@prisma/client";
...
export function AssignTrainingDialog({
  ...
  // Refetch trainings and get user position when user changes
  useEffect(() => {
    if (selectedUser) {
      fetchTrainings();

      // Get the selected user's position and its required SOPs
      const user = users.find((u) => u.id === selectedUser);
      if (user && user.positionId) {
        // Fetch the position details with its required SOPs
        fetch(`/api/sites/${siteId}/positions`)
          .then((res) => res.json())
          .then((positions) => {
            const userPosition = positions.find(
              (p: Position) => p.id === user.positionId
            );
            if (userPosition) {
  ...
  }, [selectedUser, fetchTrainings, users, siteId]);
  ...
  // SOPs required by the user's position
  const requiredSOPs = useMemo(() => {
    if (!selectedUserPosition || !selectedUserPosition.sops)
      return new Set<string>();
    return new Set(selectedUserPosition.sops.map((sop) => sop.id));
